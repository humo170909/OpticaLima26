...
function applyAuditFilters(query, filters) {
  ...
  if (filters.usuario) {
    const safeUsuario = filters.usuario.replace(/[%_\\]/g, "\\$&");
    query = query.or(`usuario.ilike.%${safeUsuario}%,correo.ilike.%${safeUsuario}%`);
  }

  if (filters.search) {
    const safeSearch = filters.search.replace(/[%_\\]/g, "\\$&");
    query = query.or(
      `usuario.ilike.%${safeSearch}%,correo.ilike.%${safeSearch}%,accion.ilike.%${safeSearch}%,registro_id.ilike.%${safeSearch}%,descripcion.ilike.%${safeSearch}%`
    );
  }
